...
class Schedule(db_tools.AbstractBaseMixin, rpc_tools.RpcMixin, db.Base):
    ...
    def run(self, debug=False):
        ...
        if self.time_to_run:
            ...
            # raising handler used to leave last_run unchanged, which made
            # time_to_run permanently true and caused the scheduler to refire
            # on every poll. Cron is the source of truth for cadence; whether
            # the handler completed cleanly does not change when the next
            # tick is due.
